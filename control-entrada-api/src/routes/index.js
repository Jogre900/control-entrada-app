...
const storage = multer.diskStorage({
  ...
  const verifyToken = (req, res, next) => {
    if(!req.headers['authorization']) return next(CreateError(401, "Usuario no Autorizado"))
...
const router = Router()
    ...
    router.post("/createVisit/:id", verifyToken(), uploadImg.array('file'), Methods.createVisits)
